...
- **Primary view (REQ-PRESENT-1):** Full screen with a single focal area that shows:
  - a **start time label** (e.g. `11:00 AM`)
  - the **current/next task title**
- **Smart Visibility (REQ-PRESENT-6):**
  - **Auto-complete items:** If an item is marked "auto-complete" and its scheduled time has passed, it should **not** be shown as the current task (assume implicitly handled). It should only be shown if it is the immediate next item and time has not passed.
  - **Next Task Logic:** The "Next: ..." indicator should prioritize the next **non-auto-complete** item (deliberate action) to give the user better foresight, skipping intervening auto-complete items if necessary.
- **Empty state (REQ-PRESENT-2):** Show something relaxing and encouraging.


- **Core Actions:**
    
    - **Time tweak (REQ-PRESENT-3):** 
      - **Mobile:** Swipe Left triggers reschedule options.
      - **Web:** Explicit buttons for interaction.
        
    - **Completion feedback (REQ-PRESENT-4):** Mark task as done.
      - **Mobile:** Swipe Right.
      - **Web:** Explicit "Complete" button.
      - **Recurring Items:** Completing a recurring task immediately spawns the next occurrence based on its repeat interval (e.g., scheduled for tomorrow).
      - SiSy may ask a lightweight follow-up (“How do you feel?”) depending on user preference and task type (Future).

    - **View Details (REQ-PRESENT-8):**
      - Clicking the task card opens a **minimalist, read-only card** showing the description.
      - **Constraint:** This view is **non-editable**. Editing task details (title, time, etc.) is explicitly reserved for the "Routine" page to maintain the "Present" page's focus on execution.

    - **Gesture mapping (REQ-PRESENT-5):** 
      - **Mobile:**
        - **Tap card:** open read-only details
        - **Swipe right:** complete current task
        - **Swipe left:** open reschedule options (Skip / Delay)
      - **Web:**
        - **Click card:** open read-only details
        - **Click buttons:** Clear actions for "Complete" (Check) and "Reschedule" (Clock).
        - **Delay/Skip:** Reschedule offers explicit options for short delays or skipping (+24h) if it's a daily task.

    - **Skip Logic (REQ-PRESENT-7):**
      - **Explicit Skip:** A consistent "Skip" action must defer the task to the next repeat time (e.g., +1 day for daily, +N days for custom intervals).

**Acceptance criteria**
- **AC-PRESENT-1:** Present never shows more than one primary “card” / focal area of content (excluding the persistent chat input).
- **AC-PRESENT-2:** Rescheduling and completion require no more than one confirmation step.
- **AC-PRESENT-3:** Interactions are platform-appropriate (Swipe on Mobile, Click on Web).
- **AC-PRESENT-4:** Auto-complete items disappear from "Now" view automatically when their time passes.
- **AC-PRESENT-5:** "Next" view shows the next significant (non-auto-complete) task.
...
- **AC-ROUTINE-3:** User can modify items via modal (Title, Time, Description, Repeat).
